...
@section('page_title', 'Edit Pertanyaan Kuesioner')
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.kuesioner.update', $kuesioner->id) }}" method="POST">
            ...
            <div style="margin-bottom:1rem;">
                ...
                <textarea id="pertanyaan" name="pertanyaan" class="form-input" rows="3" required>{{ old('pertanyaan', $kuesioner->pertanyaan) }}</textarea>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;margin-bottom:1rem;">
                <div>
                    ...
                    <select id="tipe_jawaban" name="tipe_jawaban" class="form-input" required onchange="toggleOpsi(this.value)">
                        <option value="Teks Singkat" {{ old('tipe_jawaban', $kuesioner->tipe_jawaban) === 'Teks Singkat' ? 'selected' : '' }}>Teks Singkat</option>
                        <option value="Pilihan Ganda" {{ old('tipe_jawaban', $kuesioner->tipe_jawaban) === 'Pilihan Ganda' ? 'selected' : '' }}>Pilihan Ganda</option>
                    </select>
                </div>
                <div>
                    <label for="urutan" style="display:block;margin-bottom:0.5rem;font-weight:500;">Urutan <span style="color:red;">*</span></label>
                    <input type="number" id="urutan" name="urutan" class="form-input" value="{{ old('urutan', $kuesioner->urutan) }}" required min="1">
                </div>
                <div style="display:flex;align-items:flex-end;padding-bottom:0.5rem;">
                    <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;">
                        <input type="checkbox" name="wajib" value="1" {{ old('wajib', $kuesioner->wajib) ? 'checked' : '' }}>
                        <span style="font-weight:500;">Wajib Diisi</span>
                    </label>
                </div>
            </div>

            <div id="opsi-container" style="display:{{ old('tipe_jawaban', $kuesioner->tipe_jawaban) === 'Pilihan Ganda' ? 'block' : 'none' }};margin-bottom:1.5rem;background:#f8fafc;padding:1rem;border-radius:8px;border:1px solid #e2e8f0;">
                <label style="display:block;margin-bottom:0.5rem;font-weight:500;">Opsi Jawaban</label>
                <div id="opsi-list">
                    @if(old('opsi') || $kuesioner->opsiJawaban->isNotEmpty())
                        @php $opsiList = old('opsi', $kuesioner->opsiJawaban->pluck('opsi')->toArray()); @endphp
            ...
            <div style="display:flex;gap:0.75rem;">
                <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Perbarui Pertanyaan</button>
